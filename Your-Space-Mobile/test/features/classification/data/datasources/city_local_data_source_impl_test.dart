...
void main() {
  ...
  group('applyCityChanges', () {
    test('upserts a clean row given in upserts', () async {
      await dataSource.applyCityChanges(
        upserts: const [City(id: 1, governorateId: 7, name: 'From Server')],
        tombstoneIds: const [],
      );
    ...
    test('tombstones exactly the ids given in tombstoneIds — no absence inference', () async {
      await dataSource.saveCities(const [
        City(id: 1, governorateId: 7, name: 'Untouched'),
        City(id: 2, governorateId: 7, name: 'Removed'),
      ]);

      await dataSource.applyCityChanges(upserts: const [], tombstoneIds: const [2]);

      final untouched = await (database.select(database.citiesTable)..where((t) => t.id.equals(1))).getSingle();
      expect(untouched.isDeleted, isFalse);
      final removed = await (database.select(database.citiesTable)..where((t) => t.id.equals(2))).getSingle();
    ...
    test('a dirty row is not tombstoned even if its id is given in tombstoneIds', () async {
      ...
      await dataSource.applyCityChanges(upserts: const [], tombstoneIds: const [1]);
  ...
  group('getCitiesSyncCursor / saveCitiesSyncCursor', () {
    test('reads 0 when never synced', () async {
      expect(await dataSource.getCitiesSyncCursor(), 0);
    });

    test('round-trips a saved cursor', () async {
      await dataSource.saveCitiesSyncCursor(137);

      expect(await dataSource.getCitiesSyncCursor(), 137);
    });

    test('a later save overwrites the earlier value', () async {
      await dataSource.saveCitiesSyncCursor(50);
      await dataSource.saveCitiesSyncCursor(90);

      expect(await dataSource.getCitiesSyncCursor(), 90);
    });
    ...
    test('does not clobber a previously-written lastSyncedAt', () async {
      ...
      await database.into(database.syncStateTable).insertOnConflictUpdate(
            SyncStateTableCompanion.insert(collection: 'cities', lastSyncedAt: Value(syncedAt)),
          );

      await dataSource.saveCitiesSyncCursor(137);

      final row =
          await (database.select(database.syncStateTable)..where((t) => t.collection.equals('cities'))).getSingle();
