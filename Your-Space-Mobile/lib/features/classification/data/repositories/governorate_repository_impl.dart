...
class GovernorateRepositoryImpl implements GovernorateRepository {
  ...
  Future<Either<Failure, Unit>> refreshGovernorates() async {
    ...
    // is meant to be small (27 global rows + a user's own custom ones),
    // never expected to trip.
    const maxPages = 50;
    var cursor = await _local.getGovernoratesSyncCursor();
    for (var page = 0; page < maxPages; page++) {
      final result = await _remote.getGovernorateChanges(since: cursor, pageSize: pageSize);
      if (result.isLeft()) {
        return result.fold(Left.new, (_) => throw StateError('unreachable'));
      }
      final changes = result.getOrElse(() => throw StateError('unreachable'));
      await _local.applyGovernorateChanges(
        upserts: changes.upserts.map((r) => r.toEntity()).toList(),
        tombstoneIds: changes.tombstoneIds,
      );
      await _local.saveGovernoratesSyncCursor(changes.cursor);
