...
/// Local mirror of the synced [City] entity (`lib/core/entities/city.dart`).
///
/// `governorateId` is a plain FK column — no drift-level foreign-key
/// constraint (matches `PersonsTable`'s denormalized parent-id columns);
/// `watchCities` filters on it in a drift `WHERE` clause (design doc §3).
...
/// is needed when Tier 2/3 land (design doc §4), mirroring `GovernoratesTable`.
class CitiesTable extends Table {
  ...
  // (design doc §6/§11 row 8.11); Tier 1 upserts have no value to put here yet.
