...
- `Helpers/MockDataSeeder.cs` — a static `SeedAsync(YourSpaceDbContext context)` orchestrator, empty until the first feature's entity exists. Each feature adds its own `Seed<Entity>Async(context)` method here, called from `SeedAsync`. Runs in every environment, alongside `IdentitySeeder`/`ReferenceDataSeeder` — see CLAUDE.md "Mock Seed Data." Distinct from the Identity/role bootstrap seeder only in what it seeds (mock/demo rows vs. real bootstrap data), not when it runs.
...
app: Database.Migrate() (Development, or Production when explicitly opted in — see edge case 9) → always-on seeding: IdentitySeeder.SeedAsync(...) + ReferenceDataSeeder.SeedAsync(...) + MockDataSeeder.SeedAsync(...)
   → (Development: UseOpenApi + UseSwaggerUi + UseHttpLogging)
...
`Database.Migrate()` runs before the always-on seeding block, and that seeding block runs before `UseOpenApi`/`UseSwaggerUi`/`UseHttpLogging` — so seeders and API docs never come up against a schema that isn't there yet. See edge case 9 below for why `Database.Migrate()` itself stays gated (`Development`, or `Production` only via an explicit opt-in flag).
...
11. **`MockDataSeeder` runs strictly after `Database.Migrate()`, `IdentitySeeder`, and `ReferenceDataSeeder`, in that order — but is no longer gated behind `IsDevelopment()`:** seeding before the schema exists throws; seeding before `IdentitySeeder` throws on a fresh database (roles don't exist yet for `AddToRoleAsync`); seeding before `ReferenceDataSeeder` throws on city/person seed methods that resolve "Cairo"/"Giza" by name. `MockDataSeeder` now runs unconditionally, in the same always-on block as `IdentitySeeder`/`ReferenceDataSeeder` — see CLAUDE.md "Mock Seed Data." Each per-entity `Seed<Entity>Async` method must also be independently idempotent (existence check first), because `dotnet run` re-executes this block on every local restart, and it now also runs on every `Production` restart.
