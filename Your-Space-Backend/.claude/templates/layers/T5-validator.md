...
- **Validators don't set `ErrorCode` per rule** — FluentValidation failures reach the client through `ServiceResult.ValidationError`'s fixed `"Validation.Failed"` default (CLAUDE.md Architecture rule 10), keyed on a per-field `Errors` dictionary, not a single `ErrorCode`. Don't invent one code per `RuleFor(...)`.
