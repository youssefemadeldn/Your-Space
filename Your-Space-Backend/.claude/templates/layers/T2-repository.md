---
name: T2-repository
governed-by: CLAUDE.md Architecture rules 1, 3, 8 & 12 ("Localization") · dotnet_feature_prompt.md Rule 3, Rule 8, Rule 12 & §4 · patterns/P4-hashed-verification-code.md
---
...
- **If this entity is owned by a specific user** (not shared/global reference data), every constructor above also takes an owner id and filters on it directly — never left to a controller-level check alone. See CLAUDE.md Architecture rule 12 and `dotnet_feature_prompt.md` Rule 12 for the real `PersonWithSpecs`/`GovernorateWithSpecs` precedent.
