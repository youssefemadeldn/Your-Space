using FluentAssertions;
using Microsoft.Extensions.Logging;
using Moq;
using YourSpace.Data.Entities;
using YourSpace.Data.Enums;
using YourSpace.Repository.Interfaces;
using YourSpace.Repository.Specifications;
using YourSpace.Services.Services.EventGuestService.Dtos;
using YourSpace.WebAPI.Tests.Common.MockFactories;
using EventGuestServiceImpl = YourSpace.Services.Services.EventGuestService.EventGuestService;

namespace YourSpace.WebAPI.Tests.Unit.Services.EventGuestService;

public class EventGuestService_MarkInvitedAsyncTests
{
    private readonly Mock<IUnitOfWork> _unitOfWork = new();
    private readonly Mock<IGenericRepository<EventGuest, int>> _guestRepo = new();

    public EventGuestService_MarkInvitedAsyncTests()
    {
        _unitOfWork.Setup(u => u.Repository<EventGuest, int>()).Returns(_guestRepo.Object);
    }

    private EventGuestServiceImpl CreateSut() => new(
        _unitOfWork.Object,
        MapperFactory.Create(),
        LocalizerMockFactory.Create().Object,
        Mock.Of<ILogger<EventGuestServiceImpl>>());

    [Fact]
    public async Task Returns_not_found_when_guest_does_not_exist()
    {
        _guestRepo.Setup(r => r.GetByIdWithSpecAsync(It.IsAny<ISpecification<EventGuest>>())).ReturnsAsync((EventGuest?)null);

        var result = await CreateSut().MarkInvitedAsync("owner-1", eventId: 1, guestId: 99, new MarkGuestInvitedDto { InviteMethod = InviteMethod.WhatsApp });

        result.Success.Should().BeFalse();
        result.ErrorCode.Should().Be("EventGuest.NotFound");
    }

    [Fact]
    public async Task Sets_status_method_and_timestamp()
    {
        var group = new Group { Id = 1, OwnerUserId = "owner-1", Name = "Relatives" };
        var person = new Person { Id = 10, OwnerUserId = "owner-1", Name = "Ahmed", GroupId = 1, Group = group };
        var guest = new EventGuest { Id = 1, EventId = 1, PersonId = 10, Person = person, Status = EventGuestStatus.NotInvited };
        _guestRepo.Setup(r => r.GetByIdWithSpecAsync(It.IsAny<ISpecification<EventGuest>>())).ReturnsAsync(guest);

        var result = await CreateSut().MarkInvitedAsync("owner-1", eventId: 1, guestId: 1, new MarkGuestInvitedDto { InviteMethod = InviteMethod.PhoneCall });

        result.Success.Should().BeTrue();
        guest.Status.Should().Be(EventGuestStatus.Invited);
        guest.InviteMethod.Should().Be(InviteMethod.PhoneCall);
        guest.InvitedAt.Should().NotBeNull();
    }

    [Fact]
    public async Task Re_inviting_an_already_invited_guest_just_corrects_the_method()
    {
        var invitedAt = DateTime.UtcNow.AddDays(-1);
        var guest = new EventGuest
        {
            Id = 1, EventId = 1, PersonId = 10,
            Person = new Person { Id = 10, OwnerUserId = "owner-1", Name = "Ahmed", GroupId = 1, Group = new Group { Id = 1, OwnerUserId = "owner-1", Name = "Relatives" } },
            Status = EventGuestStatus.Invited, InviteMethod = InviteMethod.WhatsApp, InvitedAt = invitedAt
        };
        _guestRepo.Setup(r => r.GetByIdWithSpecAsync(It.IsAny<ISpecification<EventGuest>>())).ReturnsAsync(guest);

        var result = await CreateSut().MarkInvitedAsync("owner-1", eventId: 1, guestId: 1, new MarkGuestInvitedDto { InviteMethod = InviteMethod.Physical });

        result.Success.Should().BeTrue();
        guest.InviteMethod.Should().Be(InviteMethod.Physical);
        guest.InvitedAt.Should().BeAfter(invitedAt);
    }
}
