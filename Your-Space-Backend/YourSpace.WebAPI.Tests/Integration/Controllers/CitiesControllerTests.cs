...
// Covers only the new flat "all mine" endpoint added in row 8.8 (doc/local-first-sync-design.md
// §11) — the existing nested CRUD routes (create/get/update/delete under
// /governorates/{id}/cities) have no integration coverage yet, same gap every Classification
// controller had before Row 8 (see GovernoratesControllerTests's own row 8.5 note); that gap is
// not this step's job to close.
