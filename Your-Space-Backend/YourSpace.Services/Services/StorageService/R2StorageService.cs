...
// s3Client is Lazy so a blank/invalid R2 config only fails the storage calls below — not the
// DI resolution of AuthService (which takes IR2StorageService for avatar upload/remove) and,
// through it, every auth endpoint. See StorageServiceExtension for the full rationale.
public class R2StorageService(Lazy<IAmazonS3> s3Client, IOptions<R2Settings> options) : IR2StorageService
{
    public async Task UploadAsync(string bucketName, string objectKey, IFormFile file, CancellationToken ct = default)
    {
        ...
        await s3Client.Value.PutObjectAsync(request, ct);
    }

    public async Task DeleteAsync(string bucketName, string objectKey, CancellationToken ct = default)
    {
        await s3Client.Value.DeleteObjectAsync(new DeleteObjectRequest { BucketName = bucketName, Key = objectKey }, ct);
    }
    ...
    public Task<string> GetPresignedUrlAsync(string bucketName, string objectKey)
    {
        ...
        return s3Client.Value.GetPreSignedURLAsync(request);
    }
}
