...
// changed for the owner (global rows + their own) since `Cursor` was last seen: upserts
// (created/updated rows) and tombstones (soft-deleted row ids) in the same page, ordered by
// SyncVersion.
public class GovernorateChangesDto
{
    public required IReadOnlyList<GovernorateProfileDto> Upserts { get; set; }
